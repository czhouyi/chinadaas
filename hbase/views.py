...
from guoDao import guo_query as guoquery
# Create your views here.

spliter = ('\x01', '|')

def hbase_query(request):
    p = request.POST
    g = request.GET
    tablename = None
    rowkey = None
    ...
    ctx = {'tablename':tablename, 'head':head, 'value':value}
    ctx.update(csrf(request))
    return render_to_response('hbase/query.html',ctx)

def hbase_tables(request):
    dao = HbaseDao()
    ctx = {'tables' : dao.getTables()}
    return render_to_response('hbase/tables.html',ctx)
